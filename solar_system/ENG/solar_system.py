...
Press the "+" or "-" keys to increase or decrease speed.
Click on the screen to turn on or off the track display.
Click on a planet to follow it.
Drag the mouse to launch the spacecraft.


Version 1.2.6 update: Added tracking spacecraft, which allows you to control the spacecraft by pressing the direction keys.
Version 1.3.1: Added the use of Tab, Shift+Tab keys to switch between planetary functions.
Version 1.3.2: Added the use of the pickle module to save celestial data and delete celestial functions.
Version 1.3.3: Fixed bugs in the function of saving celestial data and optimized code.
Add Ctrl+D keys to delete all ship functions.
"""
try:
...
class GravSys:
    ...
    def start(self): # 主循环, 最关键的函数
        ...
        while True:
            ...
            if self.show_tip:
                ...
                self.writer.clear()
                self.writer.goto(
                    scr.window_width()//2-250,scr.window_height()//2-h
                )
                self.writer.write(
                    tip,
                    font = (None,fontsize)
    ...
    def _ondrag(self,event): # 鼠标拖曳事件
        ...
        if math.hypot(x - self.startx, y - self.starty) >= 9: # 鼠标移动的距离足够大
            ...
            pen.write("Speed: {:.4g}".format(v),font = (None,11))
            pen.penup()
...
def main():
    ...
    if os.path.isfile(file):
        ...
        io_ = Star(gs,"Io", MOON_MASS, (1125,0), (0,145),
